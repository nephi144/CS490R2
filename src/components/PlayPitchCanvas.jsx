...
//
// TIME-BASED rendering — no index assumptions.
// Active note is found by: elapsedMs >= startMs && elapsedMs < endMs
// Works for any voice (SATB) melody array.
// ─────────────────────────────────────────────────────────────
...
export default function PlayPitchCanvas({
  notes,
  // activeNoteIndex is OPTIONAL — canvas derives its own from elapsedSec when not provided.
  // When provided (e.g. from PlayPage) it is used as-is for efficiency.
  activeNoteIndex: activeNoteIndexProp,
  pitchHistory,
  elapsedSec,
  height = 380,
  isPlaying = false,
  ...
  // ── Derive freq range + total duration from notes ─────────
  ...
  useEffect(() => {
    ...
    // If parent passed a pre-computed index use it; otherwise derive from time.
    const elapsedMs = elapsedSec * 1000;
    let activeNoteIndex = activeNoteIndexProp ?? -1;
    if (activeNoteIndex === undefined || activeNoteIndex === null || activeNoteIndex < 0) {
      if (notes?.length) {
        activeNoteIndex = notes.findIndex(
          (n) => elapsedMs >= n.startMs && elapsedMs < n.endMs
        );
        // Clamp to last note after melody ends
    ...
    if (notes?.length) {
      ...
      // TIME-BASED: find which note is at the clamped playhead position
      const clampedMs     = clampedSec * 1000;
      const currentNote   =
        notes.find((n) => clampedMs >= n.startMs && clampedMs < n.endMs)
    ...
    if (validPoints.length > 1) {
      ...
      // TIME-BASED active note for cents comparison
      const elapsedMs2  = elapsedSec * 1000;
      const activeN     = notes?.find(
        (n) => elapsedMs2 >= n.startMs && elapsedMs2 < n.endMs
      ) ?? null;
      const targetHz    = activeN?.freq ?? null;
      ...
      for (let i = 1; i < validPoints.length; i++) {
        ...
        const x1_   = PAD_LEFT + frac1 * (elapsedX - PAD_LEFT);
        const y0    = toY(prev.hz);
        const y1_   = toY(cur.hz);
